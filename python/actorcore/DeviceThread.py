...
class ThreadExit(Exception):
    """ Used merely to signal the .run method to exit. """
    pass
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                if qlen > 0:
                    logging.debug("%s thread has %d items after a .get()" % (self.name, qlen))

                try:
                    method = getattr(msg, 'method')
                except AttributeError as e:
                    raise AttributeError("thread %s received an unknown method: %s" % (self.name, msg.method))
                ...
                try:
                    ...
                    self._realCmd(cmd).inform('text="%s: uncaught exception running %s: %s"' % 
                                              (self.name, method, e))
