...
from ops.core.protocols.keys import Consumer,TypedValues,Key,CmdKey,KeysManager
from ops.core.protocols.keysformat import KeysFormatParser
...
class Cmd(Consumer,DispatchMixin):
	...
	def consume(self,message):
		self.trace(message)
		if not isinstance(message,messages.Command):
	...
	def create(self,*kspecs,**kwargs):
		...
		for kspec in kspecs:
			if isinstance(kspec,str):
				keywords.append(messages.Keyword(kspec))
		...
		command = messages.Command(self.verb,values,keywords)
...
class ValidationTest(unittest.TestCase):
	...
	def setUp(self):
		...
		self.c123 = messages.Command('cmd',keywords=[self.k1,self.k2,self.k3])
		self.c321 = messages.Command('cmd',keywords=[self.k3,self.k2,self.k1])
		self.c12v = messages.Command('cmd',keywords=[self.k1,self.k2],values=['1.23','0xbeef'])
		CmdKey.setKeys(KeysDictionary('<command>',self.key2,self.key3))
		self.cmd1 = Cmd('cmd','key1 <key2> <key3>')
		self.cmd2 = Cmd('cmd','@key1 <key2> [<key3>]')
		self.cmd3 = Cmd('cmd',types.Float(),types.Hex(),'(@key1 [<key2>]) [<key3>]')
	...
	def test03(self):
		from ops.core.protocols.keys import KeysError
